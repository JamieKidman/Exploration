...
download_range()
{
    ...
    do
        ...
        then
            clean_start=9999 # Both start and end are re-initalised in a way that will re trigger the loop
            clean_end=0
...
download_random()
{
    ...
    random_list=$(shuf <<<$picture_id_list) # Uses the shuf program to create a randomly ordered list       # <<< is used to pipe the function into the program
...
menu_board()
{
    ...
    sed -i "215s/^    echo/    # echo/" $0 # Inserts a # on the lines thats outputs show how to access a hidden option
    sed -i "216s/^    echo/    # echo/" $0 # Inserts a # on the lines thats outputs show how to access a hidden option
}
...
main()
{
    ...
    [[ $input = "rm" ]] && rm -rf "${output_directory}" && read -p "Input: " # hidden option that deletes files and folders created or downloaded by this script for testing and would probably help with marking
...
: '
large_testfile="http://ipv4.download.thinkbroadband.com/5MB.zip"
test_file="https://secure.ecu.edu.au/service-centres/MACSC/gallery/152/DSC01607.jpg"

#file_size=`curl https://secure.ecu.edu.au/service-centres/MACSC/gallery/152/DSC01607.jpg > file.zip` &
file_size=$( wget $large_testfile )
echo $( ${file_size} | grep -Eo 'Length: ([0-9]*)' | grep -Eo '[0-9]*' )
# | grep -Eo "\([0-9]+.{1,3}\)" | grep -Eo "\d*\w*"

echo "${file_size} words testing"
'
